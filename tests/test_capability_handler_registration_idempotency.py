...
from bot.modules import atri_capability_engine as capability
...
def test_capability_registration_is_once_per_client(repeat):
    client = _FakeClient()
    assert capability.add_capability_handlers(client) is True
    first = list(client.handlers)
    assert len(first) == 5

    for _ in range(repeat - 1):
        assert capability.add_capability_handlers(client) is False
...
def test_capability_registration_is_per_client():
    ...
    assert capability.add_capability_handlers(first) is True
    assert capability.add_capability_handlers(second) is True
