...
pub struct ArtifactIndex { db_path: PathBuf }
...
impl ArtifactIndex {
    ...
    pub fn store(&self, request: StoreRequest) -> Result<StoreResult> {
        ...
        let artifact_ref = format!("{}-{}", request.sha256.chars().take(12).collect::<String>(), short_hash(&request.chat_key));
        tx.execute("DELETE FROM artifacts WHERE chat_key=?1 AND sha256=?2", params![request.chat_key, request.sha256])?;
        tx.execute("INSERT INTO artifacts(artifact_ref,chat_key,filename,sha256) VALUES(?1,?2,?3,?4)", params![artifact_ref, request.chat_key, request.filename, request.sha256])?;
        ...
        for chunk in request.chunks.into_iter().take(2500) {
            ...
            tx.execute("INSERT INTO chunks(artifact_id,path,content,content_sha256) VALUES(?1,?2,?3,?4)", params![artifact_id, chunk.path, content, digest])?;
            count += 1;
        }
        tx.commit()?;
        Ok(StoreResult { artifact_ref, chunk_count: count })
    }
    ...
    pub fn search(&self, chat_key: &str, query: &str, limit: usize) -> Result<Vec<SearchHit>> {
        ...
        let words: Vec<String> = query.split_whitespace().map(|s| s.to_lowercase()).filter(|s| s.len() > 1).collect();
        let rows = stmt.query_map([chat_key], |r| Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?,r.get::<_,String>(2)?,r.get::<_,String>(3)?)))?;
        ...
        for row in rows {
            ...
            hits.push(SearchHit { artifact_ref, filename, path, content, score: matched as f64 / words.len().max(1) as f64 });
        }
...
fn short_hash(value: &str) -> String { hex::encode(Sha256::digest(value.as_bytes())).chars().take(6).collect() }

fn secret_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?im)^(\s*(?:api[_-]?key|token|secret|password|authorization)\s*[:=]\s*)(.+)$").unwrap())
}

pub fn redact(text: &str) -> String { secret_re().replace_all(text, "$1<REDACTED>").into_owned() }
...
mod tests {
    ...
    fn stores_and_searches() {
        ...
        let stored = index.store(StoreRequest { chat_key:"1:0".into(), filename:"x.log".into(), sha256:"abcdefabcdef".into(), chunks:vec![Chunk{path:"x.log".into(),content:"token=secret\nfatal database timeout".into()}] }).unwrap();
